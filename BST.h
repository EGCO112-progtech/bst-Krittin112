...
void insertNode( BST *b, int value ){
	...
	if(new_node)
   {
		...
		new_node->data=value;
		/*First Node*/
		if(!b->root)	b->root=new_node; //if(b->size==0)
		else 
      {
         while(!inserted)
         {
            if(t->data >=value)
            {
               ...
               if(t->leftPtr == NULL) //ซ้ายของ root ยังแทรกได้
               {
                  t->leftPtr = new_node;
                  inserted = 1;
               }
            ...
            else
            {
               ...
               if(t->rightPtr == NULL) //ขวาของ root ยังแทรกได้
               {
                  t->rightPtr = new_node;
                  inserted = 1;
               }
...
void rotateOrder( TreeNodePtr treePtr, int l)
{
   int i;
